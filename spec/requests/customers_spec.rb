...
describe "get new_customer_path" do
  ...
  describe "put customer_path with valid data" do
    it "updates an entry and redirects to the show path for the customer" do
      customer = FactoryBot.create(:customer)
      put customer_path(customer.id), params: {customer: {email: "myemail@gmail.com"} }
      customer.reload  # this is required
      expect(customer.email).to eq("myemail@gmail.com") # it validates it
      get customer_path
      expect(response).to render_template(:show)
    end
  end
  ...
  describe "delete a customer record" do
    it "deletes a customer record" do
      ...
      #after destroying something go to the /customers main page
      get customers_path
      expect(response).to render_template(:index)
    end
  end
